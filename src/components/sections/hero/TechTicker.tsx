...
export const TechTicker = () => {
  return (
    <div className="absolute bottom-0 left-0 w-full overflow-hidden border-t border-border bg-background py-6">
      <motion.div 
      ...
      >
        {[...techs, ...techs, ...techs].map((tech, i) => (
          <div key={i} className="flex items-center gap-2">
            ...
            <span className="text-muted-foreground text-sm font-bold uppercase tracking-[0.2em]">{tech.name}</span>
          </div>
        ))}
      </motion.div>
    </div>
